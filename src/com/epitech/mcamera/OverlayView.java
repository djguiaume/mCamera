...
public class OverlayView extends View  {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.d(MySurfaceView.VTAG, "draw overlay");
        if (faces != null) {
            for (Camera.Face face : faces) {
                Log.d(MySurfaceView.VTAG, face.toString());
                if (face != null && face.score > 90) {
                    canvas.drawRect(new Rect(
                            (face.rect.left + ZERO_NORMALIZE),
                            (face.rect.top + ZERO_NORMALIZE) - face.rect.height(),
                            (face.rect.right + ZERO_NORMALIZE),
                            (face.rect.bottom + ZERO_NORMALIZE) - face.rect.height()
                    ), mPaint);
                }
            }
        }
